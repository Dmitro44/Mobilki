...
#figure(
  rect(stroke: 1.5pt + black, inset: 0pt)[
    #image("img/use_case.png", width: 57%)
  ],
...
Для обеспечения работы таймера в фоновом режиме реализован сервис, который отображает постоянное уведомление, содержащее информацию о текущем состоянии таймера и элементы управления воспроизведением. Уведомление показывает название выполняемой последовательности, название и оставшееся время текущей фазы, а также индикатор прогресса. Пользователь может управлять таймером непосредственно из уведомления без необходимости открывать приложение.

Кнопки управления в уведомлении позволяют приостановить таймер, остановить его, перейти к предыдущей или следующей фазам. Каждая кнопка отправляет соответствующий сигнал сервису, который обрабатывает действие и обновляет состояние таймера. Уведомление автоматически обновляется при каждом изменении состояния, отражая актуальные данные о выполнении последовательности.

Механизм построения и обновления уведомлений обеспечивает создание канала с высоким приоритетом для видимости на всех версиях системы, формирование макета уведомления и привязку кнопок управления к соответствующим действиям. На рисунке @notificationScreen представлен интерфейс уведомления сервиса.

#figure(
  rect(stroke: 1.5pt + black, inset: 0pt)[
    #image("img/notification.png", height: 50%)
  ],
  caption: [Уведомление фонового сервиса]
) <notificationScreen>

Сервис работает в режиме, предотвращающем его принудительное завершение системой, что обеспечивает непрерывную работу таймера даже при низком уровне заряда батареи или очистке памяти. Сервис использует соответствующую категорию для приложений, не относящихся к воспроизведению мультимедиа или навигации.
